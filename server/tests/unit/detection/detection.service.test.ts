...
describe("DetectionService", () => {
  ...
  describe("Input validation", () => {
    ...
    it("should throw an error when business ID is missing", async () => {
      ...
      await expect(
        DetectionService.analyze(file, "", "shelf-123", "user-123"),
    ...
    it("should throw an error when shelf ID is missing", async () => {
      ...
      await expect(
        DetectionService.analyze(file, "business-123", "", "user-123"),
    ...
    it("should throw an error when user ID is missing", async () => {
      ...
      await expect(
        DetectionService.analyze(file, "business-123", "shelf-123", ""),
  ...
  describe("Successful scan", () => {
    it("should process AI detections and complete the scan", async () => {
      ...
      expect(mockedRepository.updateScanStatus).toHaveBeenCalledWith(
        "scan-123",
        "PROCESSING",
      );
      ...
      expect(mockedRepository.findProductByName).toHaveBeenCalledTimes(2);

      expect(mockedRepository.createDetection).toHaveBeenCalledTimes(2);

      expect(mockedRepository.updateScanStatus).toHaveBeenLastCalledWith(
        "scan-123",
        "COMPLETED",
      );
  ...
  describe("Failure handling", () => {
    it("should mark the scan as FAILED when the AI service fails", async () => {
      ...
      mockedAxios.post.mockRejectedValue(aiError);

      await expect(
        DetectionService.analyze(file, "business-123", "shelf-123", "user-123"),
