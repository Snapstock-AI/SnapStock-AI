...
describe("POST /detection/analyze", () => {
  ...
  it("should successfully process a shelf image", async () => {
    const result = {
      ...
      detections: [
        ...
        },
        {
          ...
          class_name: "Lemon",
          confidence: 0.88,
          bounding_box: { x1: 120, y1: 30, x2: 200, y2: 220 },
          freshness: "Spoiled",
    ...
    expect(mockedService.analyze).toHaveBeenCalledWith(
      expect.objectContaining({ originalname: "shelf.jpg", mimetype: "image/jpeg" }),
      "business-123", "shelf-123", "user-123", "STOCK_IN",
    );
  });

  it("should return 400 when no image is uploaded", async () => {
    mockedService.analyze.mockRejectedValue(new Error("No image uploaded."));
    const response = await request(app)
      .post("/detection/analyze")
      .field("businessId", "business-123")
      .field("shelfId", "shelf-123");
    ...
    expect(mockedService.analyze).toHaveBeenCalledWith(
      undefined, "business-123", "shelf-123", "user-123", "STOCK_IN",
    );
  });

  it("should return 400 when business ID is missing", async () => {
    mockedService.analyze.mockRejectedValue(new Error("Business ID is required."));
    const response = await request(app)
      .post("/detection/analyze")
      .field("shelfId", "shelf-123")
      .attach("file", Buffer.from("fake-image"), "shelf.jpg");
    ...
    expect(mockedService.analyze).toHaveBeenCalledWith(
      expect.objectContaining({ originalname: "shelf.jpg", mimetype: "image/jpeg" }),
      undefined, "shelf-123", "user-123", "STOCK_IN",
    );
  });

  it("should return 400 when shelf ID is missing", async () => {
    mockedService.analyze.mockRejectedValue(new Error("Shelf ID is required."));
    const response = await request(app)
      .post("/detection/analyze")
      .field("businessId", "business-123")
      .attach("file", Buffer.from("fake-image"), "shelf.jpg");
    ...
    expect(mockedService.analyze).toHaveBeenCalledWith(
      expect.objectContaining({ originalname: "shelf.jpg", mimetype: "image/jpeg" }),
      "business-123", undefined, "user-123", "STOCK_IN",
    );
  });

  it("should return 400 when the scanning service fails", async () => {
    mockedService.analyze.mockRejectedValue(new Error("AI service unavailable"));
    const response = await request(app)
      .post("/detection/analyze")
      .field("businessId", "business-123")
      .field("shelfId", "shelf-123")
